...
#if 0
static const RealtekStream::Properties STREAM_DEFINITIONS[] = {
#if 0
    // Headphones
    // Converter (3) -> Mixer (13) -> Pin Complex (33)
    { .stream_id           = 1,
      .conv_nid            = 3,
      .pc_nid              = 33,
      .is_input            = false,
...
#if 1
    // Speakers
    // Converter (2) -> Mixer (12) -> Pin Complex (20)
    { .stream_id           = 2,
      .conv_nid            = 2,
      .pc_nid              = 33,
      .is_input            = false,
...
mx_status_t RealtekCodec::Start() {
...
#if 0
    // Start by making sure that all of the individual Widgets in the codec are
    // in D3HOT, but the function group itself is set to D0.
    //
    // TODO(johngro) : Consider shifting this to a ref-counted system where the
    // function group is also held in D3HOT if there are no streams in use.  It
    // is unclear if there are power savings to be had here.
    static const CommandListEntry START_CMDS[] = {
        ...
        { 20u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Output (pres det)
        { 23u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Output
        { 24u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Input  (pres det)
        { 25u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Input  (pres det)
        { 26u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Input  (pres det)
        { 27u, SET_POWER_STATE(HDA_PS_D3HOT) }, // In/Out (pres det, headphone)
        { 30u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Output (pres det)
        { 33u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Output (pres det, headphone)
        { 20u, SET_POWER_STATE(HDA_PS_D3HOT) }, // Output (pres det)
        { 27u, SET_POWER_STATE(HDA_PS_D0) }, // In/Out (pres det, headphone)
        { 27u, SET_AMPLIFIER_GAIN_MUTE(false, 0, false, true) },
#endif
        { 33u, SET_PIN_WIDGET_CTRL(0xc0) },
        { 33u, SET_EAPD_BTL_ENABLE(EAPD_BTL_POWER_AMP) },

        // Routing.
        //{ 33u, SET_CONNECTION_SELECT_CONTROL(1u) }, // Headphones

        // Top level Audio Function group.
    ...
    for (const auto& cmd : START_CMDS) {
        ...
        if (res != NO_ERROR) {
            LOG("Failed to send startup command (nid %hu verb 0x%05x) (res %d)\n",
                cmd.nid, cmd.verb.val, res);
            return ERR_NO_MEMORY;
        }
    }

    for (const auto& sd : STREAM_DEFINITIONS) {
        AllocChecker ac;
        auto output = mxtl::AdoptRef(new (&ac) RealtekStream(sd));
        if (!ac.check()) {
            LOG("Failed to allocate memory for %s stream id #%u!",
                 sd.is_input ? "input" : "output", sd.stream_id);
            return ERR_NO_MEMORY;
        }
        ...
        if (res != NO_ERROR) {
            LOG("Failed to activate %s stream id #%u (res %d)!",
                 sd.is_input ? "input" : "output", sd.stream_id, res);
            return res;
        }
    }
#endif
