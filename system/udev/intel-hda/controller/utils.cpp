...
mx_status_t WaitCondition(mx_time_t timeout,
                          ...
                          void* cond_ctx) {
    DEBUG_ASSERT(poll_interval != MX_TIME_INFINITE);
...
mx_status_t GetVMORegionInfo(const mx::vmo& vmo,
    ...
    *num_regions_inout = 0;
    ...
    // Determine the size of the VMO.  It may not be a full page in length,
    // depending on what the user requested when creating it.
    uint64_t vmo_size;
...
mx_status_t ContigPhysMem::Map() {
    ...
    DEBUG_ASSERT((res == NO_ERROR) == (virt_ != 0u));
    return res;
}

void ContigPhysMem::Release() {
    if (virt_ != 0) {
        DEBUG_ASSERT(actual_size_ != 0);
