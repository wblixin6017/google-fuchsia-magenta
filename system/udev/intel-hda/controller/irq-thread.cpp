...
mxtl::RefPtr<IntelHDACodec> IntelHDAController::GetCodec(uint id) {
    DEBUG_ASSERT(id < countof(codecs_));
...
void IntelHDAController::SnapshotRIRB() {
    ...
    DEBUG_ASSERT(rirb_ && rirb_entry_count_ && rirb_mask_);
...
void IntelHDAController::ProcessRIRB() {
    mxtl::AutoLock rirb_lock(&rirb_lock_);
    DEBUG_ASSERT(rirb_snapshot_cnt_ < HDA_RIRB_MAX_ENTRIES);
    DEBUG_ASSERT(rirb_snapshot_cnt_ < rirb_entry_count_);
...
void IntelHDAController::ComputeCORBSpaceLocked() {
    DEBUG_ASSERT(corb_entry_count_ && corb_mask_);
    DEBUG_ASSERT(corb_wr_ptr_ == REG_RD(&regs_->corbwp));
    ...
    DEBUG_ASSERT(corb_entry_count_   >  corb_used);
    DEBUG_ASSERT(corb_max_in_flight_ >= corb_used);
