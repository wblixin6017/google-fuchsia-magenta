...
ihda_codec_protocol_t IntelHDACodec::CODEC_PROTO_THUNKS = {
    .get_driver_channel = [](mx_device_t* codec_dev, mx_handle_t* channel_out) -> mx_status_t {
                        DEBUG_ASSERT(codec_dev && codec_dev->ctx);
                        return DEV->CodecGetDriverChannel(channel_out);
                    },
};
#undef DEV
...
mxtl::RefPtr<IntelHDACodec> IntelHDACodec::Create(IntelHDAController& controller,
                                                      uint8_t codec_id) {
    DEBUG_ASSERT(codec_id < HDA_MAX_CODECS);
...
mx_status_t IntelHDACodec::Startup() {
    DEBUG_ASSERT(state_ == State::PROBING);
...
void IntelHDACodec::SendCORBResponse(const mxtl::RefPtr<DriverChannel>& channel,
...
void IntelHDACodec::ProcessSolicitedResponse(const CodecResponse& resp,
    ...
    if (state_ == State::PROBING) {
        ...
        DEBUG_ASSERT(probe_rx_ndx_ < countof(PROBE_COMMANDS));
...
void IntelHDACodec::ProcessUnsolicitedResponse(const CodecResponse& resp) {
    ...
    mxtl::RefPtr<DriverChannel> codec_driver_channel;
    {
...
void IntelHDACodec::FinishShutdown() {
    DEBUG_ASSERT(state_ == State::SHUTTING_DOWN);
    state_ = State::SHUT_DOWN;
}
...
mx_status_t IntelHDACodec::ProcessClientRequest(DriverChannel& channel,
...
mx_status_t IntelHDACodec::ProcessGetIDs(DriverChannel& channel, const GetIDsReq& req) {
    GetIDsResp resp;
    mx_status_t res;
...
mx_status_t IntelHDACodec::ProcessSendCORBCmd(DriverChannel& channel, const SendCORBCmdReq& req) {
    ...
    mxtl::RefPtr<DriverChannel> chan_ref = (req.hdr.cmd & IHDA_NOACK_FLAG)
                                         ? nullptr
...
mx_status_t IntelHDACodec::ProcessRequestStream(DriverChannel& channel,
                                                const RequestStreamReq& req) {
...
mx_status_t IntelHDACodec::ProcessReleaseStream(DriverChannel& channel,
                                                const ReleaseStreamReq& req) {
...
mx_status_t IntelHDACodec::ProcessSetStreamFmt(DriverChannel& channel, const SetStreamFmtReq& req) {
    // Only the dedicated driver channel is permitted to release DMA streams.
    if (channel.owner_ctx() != DRIVER_CHANNEL_ID) {
        DEBUG_LOG("RequestStream not allowed from unprivledged connections!\n");
        return ERR_ACCESS_DENIED;
    ...
    DEBUG_ASSERT(req.stream_id == stream->id());
    auto stream_channel = DriverChannelAllocator::New(stream->id());
    ...
    DEBUG_ASSERT(client_channel.is_valid());
    SetStreamFmtResp resp;
    resp.hdr = req.hdr;
...
void IntelHDACodec::NotifyChannelDeactivated(const DriverChannel& channel) {
    ...
    if (channel.owner_ctx() == DRIVER_CHANNEL_ID) {
        {
            ...
            DEBUG_ASSERT(&channel == codec_driver_channel_.get());
...
mx_status_t IntelHDACodec::CodecGetDriverChannel(mx_handle_t* channel_out) {
    if (!channel_out)
        return ERR_INVALID_ARGS;
    ...
    *channel_out = MX_HANDLE_INVALID;
    ...
    // and activate it.
    mxtl::RefPtr<DriverChannel> activate_me;
    {
        mxtl::AutoLock lock(&codec_driver_channel_lock_);
        ...
        codec_driver_channel_ = DriverChannelAllocator::New(DRIVER_CHANNEL_ID);
        if (codec_driver_channel_ == nullptr)
            return ERR_NO_MEMORY;
    ...
    DEBUG_ASSERT(activate_me != nullptr);
    mx::channel client_channel;
...
mxtl::RefPtr<IntelHDAStream> IntelHDACodec::GetStreamForChannel(const DriverChannel& channel,
