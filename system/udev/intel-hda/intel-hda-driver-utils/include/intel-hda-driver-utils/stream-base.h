...
public:
    ...
    mx_status_t ProcessSendCORBCmd  (const SendCORBCmdResp& resp)   TA_EXCL(obj_lock_);
    mx_status_t ProcessRequestStream(const RequestStreamResp& resp) TA_EXCL(obj_lock_);
    mx_status_t ProcessSetStreamFmt (const SetStreamFmtResp& resp,
...
protected:
    ...
    virtual mx_status_t BeginChangeStreamFormatLocked(const Audio2StreamSetFmtReq& fmt)
...
private:
    ...
    mx_status_t DoSetStreamFormatLocked(const Audio2StreamSetFmtReq& fmt) TA_REQ(obj_lock_);
    ...
    static mx_status_t EncodeStreamFormat(const Audio2StreamSetFmtReq& fmt,
