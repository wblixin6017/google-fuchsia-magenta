...
mx_status_t IntelHDAStreamBase::Activate(const mxtl::RefPtr<DriverChannel>& codec_channel) {
    DEBUG_ASSERT(codec_channel != nullptr);
...
void IntelHDAStreamBase::Deactivate() {
    ...
    ShutdownDriverChannels();

    {
        mxtl::AutoLock obj_lock(&obj_lock_);
        DEBUG_ASSERT(stream_channel_ == nullptr);
...
mx_status_t IntelHDAStreamBase::ProcessSendCORBCmd(const SendCORBCmdResp& resp) {
    return NO_ERROR;
}

mx_status_t IntelHDAStreamBase::ProcessRequestStream(const RequestStreamResp& resp) {
    mxtl::AutoLock obj_lock(&obj_lock_);
    mx_status_t res;
...
mx_status_t IntelHDAStreamBase::ProcessSetStreamFmt(const SetStreamFmtResp& codec_resp,
                                                    mx::channel&& ring_buffer_channel) {
    DEBUG_ASSERT(ring_buffer_channel.is_valid());
...
// TODO(johngro) : Refactor this; this packing of parameters is 95% the same
...
mx_status_t IntelHDAStreamBase::SetDMAStreamLocked(uint16_t id, uint8_t tag) {
    ...
    DEBUG_ASSERT((dma_stream_id_  == IHDA_INVALID_STREAM_ID) ==
                 (dma_stream_tag_ == IHDA_INVALID_STREAM_TAG));
...
mx_status_t IntelHDAStreamBase::DoSetStreamFormatLocked(const Audio2StreamSetFmtReq& fmt) {
    SetStreamFmtReq req;
    ...
    if (res != NO_ERROR) {
        DEBUG_LOG("Failed to encode stream format %u:%hu:%s (res %d)\n",
                  ...
                  fmt.channels,
                  Audio2BitPackingToString(fmt.packing),
                  res);
        goto send_fail_response;
    }
    ...
    // returning an error.
    DEBUG_ASSERT(codec_channel_ != nullptr);
...
mx_status_t IntelHDAStreamBase::ProcessChannel(DriverChannel& channel,
    ...
    DEBUG_ASSERT(&channel == stream_channel_.get());

    union {
        Audio2CmdHdr          hdr;
        Audio2StreamSetFmtReq set_format;
...
void IntelHDAStreamBase::NotifyChannelDeactivated(const DriverChannel& channel) {
    mxtl::AutoLock obj_lock(&obj_lock_);
...
mx_status_t IntelHDAStreamBase::EncodeStreamFormat(const Audio2StreamSetFmtReq& fmt,
                                                   uint16_t* encoded_fmt_out) {
    DEBUG_ASSERT(encoded_fmt_out != nullptr);
    ...
    // Next determine the bit packing format
    uint32_t packing;
    switch (fmt.packing) {
    case AUDIO2_BIT_PACKING_8BIT:          packing = 0; break;
    case AUDIO2_BIT_PACKING_16BIT_LE:      packing = 1; break;
    case AUDIO2_BIT_PACKING_20BIT_IN32_LE: packing = 2; break;
    case AUDIO2_BIT_PACKING_24BIT_IN32_LE: packing = 3; break;
    case AUDIO2_BIT_PACKING_32BIT_LE:      packing = 4; break;
    ...
    for (const auto& r : RATE_ENCODINGS) {
        if (r.rate == fmt.frames_per_second) {
            *encoded_fmt_out = static_cast<uint16_t>(r.encoded | channels | (packing << 4));
            return NO_ERROR;
        }
    }
