...
mx_status_t IntelHDACodecDriverBase::Bind(mx_driver_t* driver, mx_device_t* codec_dev) {
    ...
    DEBUG_ASSERT((codec_driver_ == nullptr) == (codec_device_ == nullptr));
    ...
    // Allocate a DriverChannel object which we will use to talk to the codec device
    mxtl::RefPtr<DriverChannel> device_channel = DriverChannelAllocator::New(1);
...
mx_status_t IntelHDACodecDriverBase::ProcessChannel(DriverChannel& channel,
...
void IntelHDACodecDriverBase::NotifyChannelDeactivated(const DriverChannel& channel) {
...
mx_status_t IntelHDACodecDriverBase::SendCodecCommand(uint16_t  nid,
                                                      CodecVerb verb,
                                                      bool      no_ack) {
    mxtl::RefPtr<DriverChannel> device_channel;
    {
        mxtl::AutoLock device_channel_lock(&device_channel_lock_);
