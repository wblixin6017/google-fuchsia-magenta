...
// we keep this in the usb_protocol_data_t.controller_reserved for xhci->current_txn
typedef struct {
    mx_off_t    vmo_offset;  // current offset in the VMO
    size_t      remaining;  // length of remaining data to transfer
    mx_handle_t vmo_handle;
    uint8_t     ep_index;
    uint8_t     ep_type;
    uint8_t     direction;
    bool        needs_data_event : 1;   // true if we still need to queue data event TRB
    bool        needs_status : 1;       // true if we still need to queue status TRB
} xhci_transfer_state_t;
static_assert(sizeof(xhci_transfer_state_t) <= 3 * sizeof(uint64_t), "");
...
static mx_status_t xhci_start_transfer_locked(xhci_t* xhci, xhci_endpoint_t* ep, iotxn_t* txn) {
    ...
    xhci_transfer_state_t* state = (xhci_transfer_state_t *)&proto_data->controller_reserved;

    iotxn_get_vmo(txn, &state->vmo_handle, &state->vmo_offset);
    state->vmo_offset += txn->offset;
...
static mx_status_t xhci_get_phys(xhci_transfer_state_t* state, mx_paddr_t* out_phys,
                                 size_t* out_phys_length) {
    mx_paddr_t phys;
    mx_handle_t handle = state->vmo_handle;
    mx_off_t offset = state->vmo_offset;
    size_t length = state->remaining;
    if (length > XHCI_MAX_DATA_BUFFER) length = XHCI_MAX_DATA_BUFFER;

    mx_off_t aligned_offset = offset & ~(PAGE_SIZE - 1);
    mx_off_t align_diff = offset - aligned_offset;
    mx_status_t status = mx_vmo_op_range(handle, MX_VMO_OP_LOOKUP, aligned_offset, PAGE_SIZE,
                                         &phys, sizeof(phys));
    if (status != NO_ERROR) return status;

    *out_phys = phys + align_diff;
    size_t phys_length = PAGE_SIZE - align_diff;

    size_t remaining = length - phys_length;
    while (remaining > 0) {
        mx_paddr_t next_phys;
        status = mx_vmo_op_range(handle, MX_VMO_OP_LOOKUP, aligned_offset, PAGE_SIZE, &next_phys,
                                 sizeof(next_phys));
        if (status != NO_ERROR) return status;
        if (next_phys == phys + PAGE_SIZE) {
            phys_length += PAGE_SIZE;
            remaining -= PAGE_SIZE;
            phys = next_phys;
        } else {
            // hit a discontinuity in physical address space
            break;
        }
    }

    if (phys_length > length) phys_length = length;
    *out_phys_length = phys_length;
...
static mx_status_t xhci_continue_transfer_locked(xhci_t* xhci, xhci_endpoint_t* ep, iotxn_t* txn) {
    ...
    while (state->remaining > 0 && free_trbs > 0) {
        ...
        size_t phys_length;
        mx_status_t status = xhci_get_phys(state, &phys_addr, &phys_length);
        if (status != NO_ERROR) return status;
        ...
        uint32_t td_size = 0; // FIXME!!! data_packets - i - 1;
        ...
        state->vmo_offset += transfer_size;
