...
static bool verity_check_all(verity_device_t* device, lba_t* off, lba_t max) {
    ...
    lba_t i = first;
    mtx_lock(&device->bitmap_mtx);
    lba_t count = __builtin_clz(~device->bitmap[i++] << (*off % VERITY_DIGESTS_PER_BLOCK));
    lba_t total = count;
    if (count == 64) {
        total -= *off;
    }
    while (count == 64 && total < max) {
...
static int verity_digester_thread(void* args) {
    ...
    while (true) {
        ...
        for (off = start; off < end; ++off) {
            ...
            prev->ops->copyfrom(prev, &block, VERITY_BLOCK_SIZE, (off - start) * VERITY_BLOCK_SIZE);
            SHA256_Init(&ctx);
            SHA256_Update(&ctx, &data);
            SHA256_Final(&ctx, &digest2);
            if (memcmp(digest1, digest2, VERITY_DIGEST_LEN) != 0) {
...
static mx_status_t verity_start(verity_device_t* device) {
    assert(device->mode == VERITY_STARTUP);
