...
static mx_status_t pci_map_resource(mx_device_t* dev,
    ...
    if (cache_policy > PCI_CACHE_POLICY_BUS_DRIVER) {
        return ERR_INVALID_ARGS;
    }
    ...
    // Set a policy if one hasn't already been set bygg the PCI bus driver
    if (cache_policy != PCI_CACHE_POLICY_BUS_DRIVER) {
...
static mx_status_t pci_get_config(mx_device_t* dev, mx_pci_resource_t* out_config) {
    if (!dev || !out_config) {
        return ERR_INVALID_ARGS;
    }
