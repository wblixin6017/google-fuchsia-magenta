...
static mx_status_t hid_open_device(void* ctx, mx_device_t** dev_out, uint32_t flags) {
    ...
    mx_status_t status = device_create("hid", inst, &hid_instance_proto, hid->drv, &inst->mxdev);
    if (status != NO_ERROR) {
...
static mx_status_t hid_bind(mx_driver_t* driver, mx_device_t* parent, void** cookie) {
    ...
    MX_DEBUG_ASSERT(hiddev->num_reports == 0);
    MX_DEBUG_ASSERT(hiddev->rbuf == NULL);
    MX_DEBUG_ASSERT(hiddev->rbuf_size == 0);
    MX_DEBUG_ASSERT(hiddev->rbuf_needed == 0);
