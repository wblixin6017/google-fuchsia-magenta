...
static mx_status_t hid_open_device(void* ctx, mx_device_t** dev_out, uint32_t flags) {
    ...
    status = device_create("hid", inst, &hid_instance_proto, hid->drv, &inst->mxdev);
    if (status != NO_ERROR) {
...
static void hid_proto_release_device(void* ctx) {
    mx_hid_device_t* hid = ctx;
...
mx_status_t hid_add_device_etc(mx_driver_t* drv, mx_hid_device_t* dev, mx_device_t* parent, const char* name) {
    ...
    char _name[MX_DEVICE_NAME_MAX + 1];
    if (name == NULL) {
        snprintf(_name, sizeof(_name), "hid-device-%03d", dev->dev_num);
    } else {
        snprintf(_name, sizeof(_name), "%s", name);
    }
    status = device_create(_name, dev, &hid_device_proto, drv, &dev->mxdev);
    if (status != NO_ERROR) {
        return status;
    }
    ...
    device_set_protocol(dev->mxdev, MX_PROTOCOL_INPUT, NULL);
