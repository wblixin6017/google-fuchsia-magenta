...
static void test_channel_call_thread_fn(void* arg_) {
    struct channel_call_suspend_test_arg* arg = arg_;

    uint8_t send_buf[8] = "abcdefgh";
    uint8_t recv_buf[8];
    uint32_t actual_bytes, actual_handles;
    mx_status_t read_status = ERR_BAD_STATE;
...
static bool test_suspend_channel_call(void) {
    ...
    ASSERT_EQ(mx_object_wait_one(thread_arg.channel, MX_CHANNEL_READABLE, 0, NULL),
              ERR_TIMED_OUT, "");
    ...
    ASSERT_EQ(mx_channel_read(thread_arg.channel, 0, buf, sizeof(buf), &actual_bytes, NULL, 0,
                              NULL),
              ERR_SHOULD_WAIT, "");
    ...
    ASSERT_EQ(mx_object_wait_one(thread_h, MX_THREAD_SIGNALED, MX_TIME_INFINITE, NULL),
              NO_ERROR, "");
    ASSERT_EQ(thread_arg.call_status, NO_ERROR, "");
