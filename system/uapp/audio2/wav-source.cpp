...
mx_status_t WAVSource::Initialize(const char* filename) {
    ...
    switch (wav_info.bits_per_sample) {
        case 8:  audio_format.sample_format = AUDIO2_BIT_PACKING_8BIT; break;
        case 16: audio_format.sample_format = AUDIO2_BIT_PACKING_16BIT_LE; break;
        default:
    ...
    // TODO(johgro) : The DATA chunk does not need to come immediately after the
    // WAVE header (but it usually does).  We should proably read and skip
    // chunks until we find the DATA chunk.
    RIFFChunkHeader data_hdr;
    res = Read(&data_hdr, sizeof(data_hdr));
    if (res != NO_ERROR) {
        printf("Failed to read DATA chunk header\n");
        return res;
    }

    if (fetch_fourcc(&data_hdr.four_cc) != DATA_FOUR_CC) {
        printf("Missing expected 'DATA' 4CC (expected 0x%08x got %08x)\n",
               DATA_FOUR_CC, riff_hdr.four_cc);
        return ERR_INVALID_ARGS;
    }

    if ((data_hdr.length + sizeof(WAVHeader) + sizeof(RIFFChunkHeader)) != riff_hdr.length) {
        printf("Header length sanity check failure (%u + %zu != %u)\n",
               data_hdr.length,
...
mx_status_t WAVSource::PackFrames(void* buffer, uint32_t buf_space, uint32_t* out_packed) {
    ...
    DEBUG_ASSERT(payload_played_ < payload_len_);
