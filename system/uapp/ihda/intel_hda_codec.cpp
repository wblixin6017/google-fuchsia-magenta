...
static mx_status_t ParseAWPcmSizeRate(AudioWidgetState& widget, const CodecResponse& resp) {
    ...
    if (caps.format_override()) widget.pcm_size_rate_ = resp.data;
    else                        widget.pcm_size_rate_ = afg.default_pcm_size_rate_;
...
static mx_status_t ParseAWPcmFormats(AudioWidgetState& widget, const CodecResponse& resp) {
    ...
    if (caps.format_override()) widget.pcm_formats_ = resp.data;
    else                        widget.pcm_formats_ = afg.default_pcm_formats_;
...
static mx_status_t ParseAWInputAmpCaps(AudioWidgetState& widget, const CodecResponse& resp) {
    ...
    if (caps.input_amp_present()) {
        if (caps.amp_param_override()) widget.input_amp_caps_ = AmpCaps(resp.data);
        else                           widget.input_amp_caps_ = afg.default_input_amp_caps_;
    }
...
static mx_status_t ParseAWOutputAmpCaps(AudioWidgetState& widget, const CodecResponse& resp) {
    ...
    if (caps.output_amp_present()) {
        if (caps.amp_param_override()) widget.output_amp_caps_ = AmpCaps(resp.data);
        else                           widget.output_amp_caps_ = afg.default_output_amp_caps_;
    }
...
mx_status_t IntelHDACodec::ReadFunctionGroupState(FunctionGroupStatePtr& ptr, uint16_t nid) {
    ...
    // We do not support probing the state of modem or vendor function groups
    // right now.
    default:
        break;
    }
...
mx_status_t IntelHDACodec::ReadAudioWidgetState(AudioWidgetState& widget) {
    ...
    if (widget.caps_.has_power_ctl()) {
        ...
        if (widget.power_.supported_states_ == 0) {
            DEBUG_ASSERT(widget.afg_ != nullptr);
...
mx_status_t IntelHDACodec::ReadConnList(AudioWidgetState& widget) {
    ...
    DEBUG_ASSERT(widget.conn_list_len_ > 0);
    DEBUG_ASSERT(widget.conn_list_ != nullptr);
