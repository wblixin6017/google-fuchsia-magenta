...
// Don't allow this code to pollute the global namespace.
namespace {
...
static void ihda_dump_codec_fn_group(const CodecState& codec, uint32_t id) {
    DEBUG_ASSERT(codec.fn_groups_ && (id < codec.fn_group_count_) && codec.fn_groups_[id]);
    static const char* pad = "+--- ";
