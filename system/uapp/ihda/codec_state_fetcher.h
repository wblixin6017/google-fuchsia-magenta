...
public:
    ...
    FunctionGroupStatePtr& get_fn_group_ptr() const {
        DEBUG_ASSERT(codec_.fn_groups_ != nullptr);
        DEBUG_ASSERT(fn_group_iter_ < codec_.fn_group_count_);
    ...
    AudioFunctionGroupState& get_afg() const {
        ...
        DEBUG_ASSERT(ptr->type_ == FunctionGroupState::Type::AUDIO);
        return *(static_cast<AudioFunctionGroupState*>(ptr.get()));
    }

    AudioWidgetStatePtr& get_widget_ptr() const {
        auto& afg = get_afg();
        DEBUG_ASSERT(afg.widgets_ != nullptr);
        DEBUG_ASSERT(widget_iter_ < afg.widget_count_);
