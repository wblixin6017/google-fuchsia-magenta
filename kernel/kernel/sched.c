...
__NO_INLINE static void boost_thread(thread_t *t)
{
    if (NO_BOOST)
        return;
    ...
    if (likely(t->priority_boost < MAX_PRIORITY_ADJ) &&
...
__NO_INLINE static void deboost_thread(thread_t *t)
{
    if (NO_BOOST)
        return;
    ...
    if (likely(t->priority_boost > -MAX_PRIORITY_ADJ) &&
       likely((t->base_priority + t->priority_boost) > LOWEST_PRIORITY)) {
        t->priority_boost--;
    }
}
