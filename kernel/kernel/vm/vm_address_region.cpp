...
status_t VmAddressRegion::Protect(vaddr_t base, size_t size, uint arch_mmu_flags) {
    DEBUG_ASSERT(magic_ == kMagic);
    ...
    for (auto itr = begin; itr != end; ++itr) {
        ...
        if (!itr->is_valid_mapping_flags(arch_mmu_flags)) {
            return ERR_ACCESS_DENIED;
        }
...
namespace {
    ...
    uint NonCompactRandomizedAllocatorSelectWidth(size_t aspace_size, size_t alloc_size) {
        ...
        // Cap log2 at 31 bits, since musl assumes 31-bit thread IDs and uses
        // thread struct addresses for those.
        return mxtl::min(log2, 31u);
    }
} // namespace
...
vaddr_t VmAddressRegion::NonCompactRandomizedRegionAllocatorLocked(size_t size, uint8_t align_pow2,
    ...
    const uint aslr_width = NonCompactRandomizedAllocatorSelectWidth(size_, size);
    ...
    const auto choose_offset = [this, align, aslr_width]() -> size_t {
        ...
        aspace_->AslrDrawLocked(reinterpret_cast<uint8_t*>(&chosen_offset), sizeof(chosen_offset));
    ...
    for (size_t tries_remaining = 10; tries_remaining > 0; --tries_remaining) {
        ...
        DEBUG_ASSERT(chosen_end <= base_ + size_);
...
vaddr_t VmAddressRegion::CompactRandomizedRegionAllocatorLocked(size_t size, uint8_t align_pow2,
    ...
    bool alloc_before;
    uint8_t num_gap_pages;
    {
        uint8_t entropy;
        aspace_->AslrDrawLocked(&entropy, sizeof(entropy));
        alloc_before = entropy & 1;
        num_gap_pages = static_cast<uint8_t>((entropy >> 1) + 1);
    }
