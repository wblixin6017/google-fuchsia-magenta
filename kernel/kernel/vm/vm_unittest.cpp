...
static bool vmm_alloc_smoke_test(void* context) {
    ...
    void* ptr;
    auto err = vmm_alloc(vmm_get_kernel_aspace(), "test", alloc_size, &ptr, 0, 0, 0, kArchRwFlags);
...
static bool vmm_alloc_zero_size_fails(void* context) {
    ...
    void* ptr;
    status_t err =
        vmm_alloc(vmm_get_kernel_aspace(), "test", zero_size, &ptr, 0, 0, 0, kArchRwFlags);
...
static bool vmm_alloc_contiguous_missing_flag_commit_fails(void* context) {
    ...
    void* ptr;
    status_t err = vmm_alloc_contiguous(vmm_get_kernel_aspace(), "test", 4096, &ptr, 0, 0,
...
static bool vmm_alloc_contiguous_zero_size_fails(void* context) {
    ...
    const size_t zero_size = 0;
    void* ptr;
    status_t err = vmm_alloc_contiguous(vmm_get_kernel_aspace(), "test", zero_size, &ptr, 0, 0,
                                        VMM_FLAG_COMMIT, kArchRwFlags);
    EXPECT_EQ(ERR_INVALID_ARGS, err, "");
    END_TEST;
}
...
static bool vmo_precommitted_map_test(void* context) {
    ...
    void* ptr;
    auto ret = ka->MapObject(vmo, "test", 0, alloc_size, &ptr, 0, 0, VMM_FLAG_COMMIT, kArchRwFlags);
...
static bool vmo_demand_paged_map_test(void* context) {
    ...
    auto ka = VmAspace::kernel_aspace();
    void* ptr;
    auto ret = ka->MapObject(vmo, "test", 0, alloc_size, &ptr, 0, 0, 0, kArchRwFlags);
...
static bool vmo_remap_test(void* context) {
    ...
    // map it again
    ret = ka->MapObject(vmo, "test", 0, alloc_size, &ptr, 0, 0, VMM_FLAG_COMMIT, kArchRwFlags);
    EXPECT_EQ(ret, NO_ERROR, "mapping object");
...
static bool vmo_double_remap_test(void* context) {
    ...
    auto ka = VmAspace::kernel_aspace();
    void* ptr;
    auto ret = ka->MapObject(vmo, "test0", 0, alloc_size, &ptr, 0, 0, 0, kArchRwFlags);
    EXPECT_EQ(NO_ERROR, ret, "mapping object");
    ...
    // map it again
    void* ptr2;
    ret = ka->MapObject(vmo, "test1", 0, alloc_size, &ptr2, 0, 0, 0, kArchRwFlags);
...
static bool vmo_read_write_smoke_test(void* context) {
    ...
    auto ka = VmAspace::kernel_aspace();
    uint8_t* ptr;
    err = ka->MapObject(vmo, "test", 0, alloc_size, (void**)&ptr, 0, 0, 0, kArchRwFlags);
