...
static struct int_handler_struct int_handler_table_shared[GIC_MAX_INT-GIC_MAX_PER_CPU_INT];
...
static void gic_register_int_handler(unsigned int vector, int_handler handler, void* arg)
{
...
static void gic_init(void)
{
    ...
    assert(gic_max_int <= GIC_MAX_INT);
...
static const struct pdev_interrupt_ops gic_ops = {
    ...
    .register_handler = gic_register_int_handler,
...
static void arm_gic_v3_init(mdi_node_ref_t* node, uint level) {
    ...
    mdi_each_child(node, &child) {
        switch (mdi_id(&child)) {
        case MDI_KERNEL_DRIVERS_ARM_GIC_V3_BASE_PHYS:
