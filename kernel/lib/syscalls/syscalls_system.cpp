...
static void thread_migrate_cpu(const uint target_cpuid) {
    ...
    xprintf("currently on %u, migrating to %u\n", old_cpu_id, target_cpuid);
#endif
    ...
    xprintf("previously on %u, migrated to %u\n", old_cpu_id, current_cpu_id);
}
...
mx_status_t sys_system_mexec(mx_handle_t kernel_vmo,
    ...
    if (result != NO_ERROR) {
        printf("mx_system_mexec failed to allocate identity mapped page, "
               "retcode = %d\n", result);
        return result;
    }

    xprintf("mx_system_mexec allocated identity mapped page at %p\n",
            id_page_addr);
